...
class UploadController extends Controller
{
    ...
    {   
           $image = UploadedFile::getInstancesByName("upload");
           ...
           $name = explode('/',$type);
           //如果文件夹不存在，则新建文件夹
           if (!file_exists(Yii::getAlias('@common') . $dir)) {
               FileHelper::createDirectory(Yii::getAlias('@common') . $dir, 777);
           }
           $filepath = realpath(Yii::getAlias('@common') . $dir) . '/';
           
           $filename = $pickey .'.'.$name[1];

           $file = $filepath.$filename;
           $_path = constant('ASSETS_URL').'uploads/cke/'.$filename;
           
           
           if($image[0]->saveAs($file))
           {               
    ...
    public function actionVideo()
    {
        ...
        if (Yii::$app->request->isPost) {
           ...
           if ($video[0]->saveAs($file)) {   
               ...
               Tool::getVideoLength($video[0]->tempName);
